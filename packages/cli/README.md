...
| `devex upgrade --workflow-version v0.3.3` | Update the pinned workflow ref across `devex.yaml` and `.github/workflows/devex-pr.yml` |
...
--workflow-ref       Workflow framework version tag (e.g. v0.3.3)
...
```bash
devex upgrade --workflow-version v0.3.3
devex upgrade --workflow-version v0.3.3 --dry-run  # preview changes only
```
...
workflowVersion:
  ref: v0.3.3                    # must be a semver tag or full 40-char SHA
```
