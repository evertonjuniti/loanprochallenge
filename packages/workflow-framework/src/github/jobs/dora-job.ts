...
export function buildDoraAuditJob(
  ...
  const steps: GithubStep[] = [
    ...
    },
    {
      ...
      env: {
        // On PR events, head_ref is the source branch name.
        GITHUB_HEAD_REF: "${{ github.head_ref || '' }}",
        // On push-to-main events the merge commit message embeds the source
        // branch: "Merge pull request #N from org/feature/FIN-123-..."
...
function doraComputeScript(): string {
  return [
    ...
    `// Extract Work ID from the governance-validated source branch name.`,
    `// For push-to-main events, GitHub embeds the branch in the merge commit:`,
    `//   "Merge pull request #N from org/feature/FIN-123-description"`,
    `// This is more trustworthy than the PR title or commit body, which can`,
    `// be rewritten or AI-generated by contributors.`,
    `const mergeMatch = commitMsg.match(/Merge pull request #\\d+ from [^\\/]+\\/([^\\n]+)/);`,
    `const sourceBranch = mergeMatch ? mergeMatch[1].trim() : headRef;`,
    `const branchWorkId = sourceBranch.match(/[A-Z]+-[0-9]+/)?.[0] ?? null;`,
    `const workIdFromEvents = events.find(e => e.workId)?.workId ?? null;`,
    `const workId = branchWorkId ?? workIdFromEvents;`,
