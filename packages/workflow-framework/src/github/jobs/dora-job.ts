...
function doraComputeScript(): string {
  return [
    ...
    `const workIdFromBranch = workIdMatch ? workIdMatch[0] : null;`,
    `const workIdFromEvents = events.find(e => e.workId)?.workId ?? null;`,
    `const workId = workIdFromBranch ?? workIdFromEvents;`,
    `const fullRepo = process.env.GITHUB_REPOSITORY ?? "";`,
    `const serviceName = fullRepo.includes("/") ? fullRepo.split("/")[1] : fullRepo;`,
    `const serviceDisplay = serviceName || fullRepo || "N/A";`,
    `const environments = [...new Set(events.map(e => e.environment).filter(Boolean))];`,
    `const environmentDisplay = environments.length > 0 ? environments.join(", ") : "N/A";`,
